	...
	if (buff_index % 256 == 0){
	    ...
	    for (i = 0; i < BUFFERLENGTH; i++){
	    	if (right[i].re > maxV){
              maxV = right[i].re;
	    	  indexV = i;
			  puts ("fill_Buffers: gets an indexV value");
	    	} 
	    }
		//puts ("It should get here\n");
		

		//printf("indexV is: %d ", indexV);
		//printf("indexH is: %d\n", indexH);
	}

	if (buff_index == BUFFERLENGTH) buff_index = 0;
	return;
}
...
interrupt void McBSP1_Rx_ISR()
{
	...
	//puts("WOOOOOHOOOOOOOOOOOOOOOOOOOOOOOOO");
    
	if (flag == 0) {
		//Calculating the Twiddle factor for FFT
		for( i = 0 ; i < BUFFERLENGTH/RADIX ; i++ ) 
	    {
		   // puts ("Does this how many times");
	    	/*
	    	W[i].re = 0;			//real component of W
	  	    W[i].im = 0;			//neg imag component
			*/
		...
		for( i = 0 ; i < BUFFERLENGTH ; i++ ) 
	    {
			left[i].re = 0.0;			//real component of W
	  	    left[i].im = 0.0;			//neg imag component
			right[i].re = 0.0;			//real component of W
	  	    right[i].im = 0.0;			//neg imag component
			bottom[i].re = 0.0;			//real component of W
	  	    bottom[i].im = 0.0;			//neg imag component
...
{   
	...
	if(master) {
		...
		/* srg set up for clks/8, 32 bits, fsr width = 16bits - 48kHz */
